...
| Hairstyle overlay try-on: placement stat, overlay + hair recolored by one color, nudge controls (Iteration 5 / M6) | `flows-dev/05-style-overlay.yaml` | Yes — **dev-mode only** (same reasoning/protocol as flow 04; the "style: <id> @ x,y,w,h" placement stat is `__DEV__`-gated). Loads the bundled portrait, selects Pixie Crop (updated Iteration 5R when the SVG art was retired; as of Iteration 5R round 2 there are 4 art-bearing styles — per-style placement evidence is docs/evidence/iter5r2-*), asserts the placement stat's x,y,w,h numbers exist (negative x/y allowed — the overlay legitimately extends past the photo edge when the subject's head is near the frame edge), asserts the no-face hint is absent, recolors Teal @ Bold 1.0 (recoloring both the real hair AND the overlay through the same Lab pipeline), screenshots, and exercises nudge scale-up + reset. **Iteration 5: PASSED** (1m 9s). **Iteration 5R round 2: PASSED** (1m 9s, dev client 32841af6 + Metro, 4-style registry). |
